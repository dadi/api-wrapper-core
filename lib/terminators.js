...
module.exports = function (APIWrapper) {
  ...
  APIWrapper.prototype.create = function (documents) {
    const sanitisedDocuments = documents instanceof Array
      ? documents.map(this._stripReservedProperties.bind(this))
  ...
  APIWrapper.prototype.getSignedUrl = function (parameters) {
    if (!this.inMediaCollection) {
      throw new Error('Media collection hasn\'t been selected. Did you use `.inMedia()`?')
    }

    return this._createRequestObject({
      body: parameters,
      method: 'POST',
      uri: this._buildURL({mediaSign: true})
    })
  }
